...
levels(data$Channel) 
#Channel variable has 13 levels, in which 3 of the levels were "Walk On / Unknown", "Walk-On   / Unknown" and "Walk-On / Unknown". These 3 levels are converted to a single level. 
table(data$Channel) 
...
data <- data %>% mutate(Channel = factor(gsub(pattern="[^a-zA-Z0-9]", replacement="", Channel)), 
                        ...
                        Stated.business_size = revalue(Stated.business_size, c("1 to 3"="1 to 25", "1 to 5"= "1 to 25",
                                                                               "4 to 10" = "1 to 25", "6 to 25" = "1 to 25", 
                                                                               "11 to 25"= "1 to 25")))
...
##################################### PREDICTION #################################################
#Logistic Regression model is used for predicting Qualified Leads and Leads Won
...
#performance metrics for Qualified Leads for both train and test data 
...
#performance metrics for Leads Won for both train and test data 
rbind(Stats_LeadsWonTrain, Stats_LeadsWonTest)



#Recall is low on both training and test data for both Qualified Leads and Leads Won. This is because of 
#imbalanced class problem, so one should try to balance the dataset wither by undersampling of majority class
#or oversampling of minority class.
#I have used undersampling below to deal with the problem
#Created a nearly balanced dataset(40% TRUE, 60% FALSE) by subsampling the majority class
#Run the model around 10 times and average the results.
#This approach is similar to bootstrap averaging or model averaging
...
#Follow the same process for Leads Won 
